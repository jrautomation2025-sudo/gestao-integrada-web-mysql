...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'salvar') {
    ...
    // Pega o contexto do formulário (garantia extra) ou da sessão
    $ctx_salvar = $_POST['contexto'] ?? $contexto;

    if (!empty($nome)) {
        if ($cliente_id) {
            $stmt = $pdo->prepare("UPDATE clientes SET nome=?, telefone=?, email=? WHERE id=? AND usuario_id=?");
            $stmt->execute([$nome, $telefone, $email, $cliente_id, $user_id]);
        } else {
            // INSERE COM O CONTEXTO
            $stmt = $pdo->prepare("INSERT INTO clientes (usuario_id, nome, telefone, email, contexto) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$user_id, $nome, $telefone, $email, $ctx_salvar]);
        }
    }
    header("Location: membros"); exit;
}
...
$busca = $_GET['busca'] ?? '';
$sql = "SELECT * FROM clientes WHERE usuario_id = ? AND contexto = ?"; // Filtro adicionado
$params = [$user_id, $contexto];
    ...
    <div class="container-fluid py-4 px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="page-header mb-4">
                <h2 style="font-family: 'Cinzel', serif; font-weight: 700; color: white; font-size: 1.8rem;"><i class="fas fa-users me-2 text-warning"></i> Controle de Membros</h2>
                <p class="text-warning">Cadastre aqui os membros pagantes</p>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-card border-secondary" style="background: var(--bg-card);">
                <div class="modal-header border-bottom border-secondary">
                    <h5 class="modal-title text-gold">Cadastro Novo Membro</h5>
